    ...
    if request.method=="POST":
        ...
        if password==confirm_password:
            ...
            if user_type=="seeker":
                ...
                user_tp=SeekerEducationModel.objects.create(seeker_user=user)
                user_tp=SeekerWorkModel.objects.create(seeker_user=user)
            user_tp.save()
...
def recruiterbasic(request):
    return render(request,'profile/recruiterbasic.html')
...
def seekerbasic(request):
    return render(request,'profile/seekerbasic.html')

@login_required
def seekereducation(request):
    seekeredu=SeekerEducationModel.objects.all()
    seekereduDict={
        'seekeredu':seekeredu
    }
    return render(request,'profile/seekereducation.html',seekereduDict)

@login_required
def seekerwork(request):
    seekerwork=SeekerWorkModel.objects.all()
    seekerworkDict={
        'seekerwork':seekerwork
    }
    return render(request,'profile/seekerwork.html',seekerworkDict)
